...
all_files = glob.glob(paths["data"]["interim"]["libraries"]["path"] + "/*_prepared.tsv.gz")

df = pandas.concat((pandas.read_csv(f) for f in all_files))

if (params["filter"]["mode"]):

    df = df[df.columns.str.contains(pat=params["filter"]["level"]) == params["filter"]["value"]]

else:
    print("""Great, a comprehensive library""")

df.to_csv(os.path.join(paths["data"]["interim"]["libraries"]["path"], params["output"]))
